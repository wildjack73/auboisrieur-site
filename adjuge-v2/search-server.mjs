...
function search(p) {
  ...
  const join = p.maison ? " JOIN lots l ON l.id=f.id" : "";
  if (terms.length) {
    where.push("f MATCH ?");
    params.push(terms.map(t=>`"${t}"*`).join(" "));
  }
  if (p.cat) { where.push("f.category=?"); params.push(p.cat); }
  if (p.city) { where.push("f.city=?"); params.push(p.city); }
  if (p.minp>0) { where.push("f.price>=?"); params.push(p.minp); }
  ...
  let order;
  if (p.sort==="ph") order=" ORDER BY f.price DESC";
  else if (p.sort==="pl") order=" ORDER BY f.price ASC";
  else if (p.sort==="deal") order=" ORDER BY f.score DESC, f.sdate DESC";
  else if (p.sort==="recent") order=" ORDER BY f.sdate DESC, f.score DESC";
  else order = terms.length ? " ORDER BY rnk" : " ORDER BY f.sdate DESC";
